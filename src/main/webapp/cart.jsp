...
<%@page import="com.bee.dao.ProductDao"%>
...
<body>
	...
	<div class="container">
		<div class="d-flex py-3">
			<h3>Total Price: ${(total>0)?dcf.format(total):0 } LKR</h3>
		...
		<table class="table table-loght">
			...
			<% if(cart_list != null) {
				for(Cart c:cartProduct){%>
					...
					<td><%= c.getName() %></td>
					<td><%= c.getCategory() %></td>
					<td><%= dcf.format(c.getPrice()) %> LKR</td>
					<td>
						<form action="order-now" method="post" class="form-inline">
							...
							<div class="form-group d-flex justify-content-between w-50">
								<a class="btn btn-sm btn-decre" href="quantity-inc-dec?action=dec&id=<%= c.getId()%>"><i
									...
									name="quantity" class="form-control w-50" value="<%= c.getQuanity() %>" readonly>
